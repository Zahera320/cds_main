...
def _process_page_task(args):
    ...
    Using ThreadPoolExecutor (not ProcessPoolExecutor) is intentional: the
    parent server process has CUDA already initialised; forking via
    ProcessPoolExecutor on Linux corrupts the GPU context in child processes
    and causes silent failures.  PyMuPDF releases the GIL for its C-level
    rendering, so threads still execute the DPI render concurrently.
